...
namespace ET
{
    public class FrameBuffer: Object
    {
        ...
        // 权威帧的前进
        public OneFrameInputs MoveForwardAuthority(int frame)
        {
            if (this.MaxFrame - frame <= LSConstValue.FrameCountPerSecond) // 至少留出1秒的空间
            {
                // 权威帧的前进，直接强制最大预测帧的检测数量
                this.MaxFrame = frame + LSConstValue.FrameCountPerSecond + 1;
            }
            
            OneFrameInputs oneFrameInputs = this.FrameInputs(frame);
            return oneFrameInputs;
        }
