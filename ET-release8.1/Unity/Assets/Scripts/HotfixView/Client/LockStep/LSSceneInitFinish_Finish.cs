...
namespace ET.Client
{
    ...
    public class LSSceneInitFinish_Finish: AEvent<Scene, LSSceneInitFinish>
    {
        protected override async ETTask Run(Scene clientScene, LSSceneInitFinish args)
        {
            ...
            Debug.Log("加载 Box2D 物理引擎 ----------------1");
            
            // 加载 Box2D 物理引擎
            // LSWorld lsWorld = room.LSWorld;
            Box2DControllerComponent tBoxController = room.AddComponent<Box2DControllerComponent>();
            ...
            foreach (long playerId in room.PlayerIds)
            {
                ...
                // 发送消息添加物理系统对象
                EventSystem.Instance.Publish(clientScene, new InitBox2DBody()
                {
                    LSUnitId = playerId,
            ...
            room.AddComponent<Box2DShowComponent>();
